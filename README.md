...
Match setup offers **Easy**, **Normal** (default), and **Hard** AI for both automated teams. `AIDifficultyPolicy` changes target, movement, exposure, and survival-position scoring; Normal preserves the prior values. Easy favors less precise targeting and cover use and accepts a modest exposure increase, while Hard prioritizes vulnerable targets, cover, and squad spacing more strongly. All tiers still use the same AP, movement, attack, and objective action validation. The selected tier appears in the startup log and F3 AI decision explanation.
