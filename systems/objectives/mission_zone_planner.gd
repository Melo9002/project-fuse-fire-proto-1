...
static func populate_defaults(map_data: MapData) -> void:
	map_data.set_objective_zone(&"reach", _nearest_open_cells(map_data, Vector2i(map_data.map_size.x / 2, map_data.map_size.y / 2), 4))
	map_data.set_objective_zone(&"extract", _nearest_open_cells(map_data, Vector2i(map_data.map_size.x / 2, maxi(2, map_data.map_size.y / 4)), 4))

static func find_rescue_cell(map_data: MapData) -> Vector3i:
	var cells := _nearest_open_cells(map_data, Vector2i(map_data.map_size.x / 2, map_data.map_size.y / 2), 1)
