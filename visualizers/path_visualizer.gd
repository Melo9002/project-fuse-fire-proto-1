...
# Enterprise Optimization: Pre-cached materials to avoid runtime allocation churn
...
func _ready() -> void:
	...
	# Enterprise Fallback: Auto-locate GridManager if not wired in the Inspector
	if not grid_manager:
		grid_manager = get_tree().get_first_node_in_group("grid_manager") as GridManager
		if not grid_manager:
			# Fallback path lookup based on typical scene hierarchies
...
func draw_path(path_vectors: PackedVector3Array, color: Color = path_color) -> void:
	...
	# Convert Vector3 world points back or map them to grid format for uniform mesh generation
	# (Keeping your original offset logic cleanly encapsulated)
...
# Helper to build square tile meshes efficiently
